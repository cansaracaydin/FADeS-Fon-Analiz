...
populer_fonlar = ["TCD", "MAC", "TI3", "IPJ", "AFT", "YAY", "YAS", "NNF"]
...
if st.sidebar.button("🚀 Analizi Başlat", type="primary"):
    ...
    if not secilen_fonlar:
        ...
        # --- HAZIRLIK ---
        st.info("Veriler çekiliyor, riskler hesaplanıyor ve tablolar oluşturuluyor...")
        bar = st.progress(0)
        durum = st.empty()
        ...
        try:
            ...
            if tum_veriler:
                ...
                # 4. GEÇMİŞ FİYATLAR (DÜZENLENMİŞ VERSİYON) ✅
                with tab4:
                    st.subheader("🗓️ Geçmiş Fiyat Listesi")
                    
                    # Kullanıcıya Seçenek Sunuyoruz
                    gorunum_tipi = st.radio(
                        "Tablo Görünümü Seçin:", 
                        ["📂 Fona Göre Grupla (Alt Alta)", "📊 Yan Yana (Pivot Tablo)"], 
                        horizontal=True
                    )
                    
                    if gorunum_tipi == "📂 Fona Göre Grupla (Alt Alta)":
                        # SENİN İSTEDİĞİN GÖRÜNÜM:
                        # Önce Fon Kodu (A-Z), Sonra Tarih (Yeniden Eskiye)
                        display_df = full_df[["Date", "FundCode", "FundName", "Price", "Daily_Return", "Cumulative_Return"]].copy()
                        display_df = display_df.sort_values(by=["FundCode", "Date"], ascending=[True, False])
                        ...
                        # PIVOT GÖRÜNÜM (Fiyat Karşılaştırması İçin)
                        ...
                        st.write("Fon Fiyatlarının Yan Yana Karşılaştırması:")
